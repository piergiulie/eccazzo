%% Parametri antropometrici (adatta alle tue misure!)
l1 = 0.30; % Lunghezza spalla-gomito [m]
l2 = 0.25; % Lunghezza gomito-polso [m]
F_val = 5; % Forza verticale verso il basso applicata al polso [N]

% Lettura dati
dati_errato = readtable('dati_marker_2.csv');
dati_corretto = readtable('dati_marker_21.csv');
...
% Calcolo angoli e Jacobiano per entrambi
...
F_err = repmat([0; -F_val], 1, length(t_err));
F_corr = repmat([0; -F_val], 1, length(t_corr));
...
animazione_pushdown_separata(t_err, P1_err, P2_err, P3_err, theta1_err, theta2_err, J_err, tau_err, ...
                             t_corr, P1_corr, P2_corr, P3_corr, theta1_corr, theta2_corr, J_corr, tau_corr, F_val);
...
function animazione_pushdown_separata(t1, P1_1, P2_1, P3_1, theta1_1, theta2_1, J1, tau1, ...
                                      t2, P1_2, P2_2, P3_2, theta1_2, theta2_2, J2, tau2, F_val)
    % Layout: due in alto (animazione), due in basso (grafici)
    fig = figure('Name','Animazione Pushdown + Analisi','NumberTitle','off', ...
                 'Position',[100 100 1800 900]);

    % Subplot: 2 righe x 2 colonne
    ...
    % Sposta ax3 più vicino ad ax4
    pos3 = get(ax3,'Position');
    pos4 = get(ax4,'Position');
    delta = 0.08; % spostamento verso destra
    set(ax3,'Position',[pos3(1)+delta pos3(2) pos3(3)-delta pos3(4)]);
    set(ax4,'Position',[pos4(1) pos4(2) pos4(3)-delta pos4(4)]);
    ...
    h1 = plot(ax1, [P1_1(1,1) P2_1(1,1) P3_1(1,1)], [P1_1(1,2) P2_1(1,2) P3_1(1,2)], 'ro-','LineWidth',2,'MarkerFaceColor','r');
    ...
    minx = min(allx)-0.05; maxx = max(allx)+0.05;
    miny = min(ally)-0.05; maxy = max(ally)+0.05;
    set(ax1,'XLim',[minx maxx],'YLim',[miny maxy]);
    hArc1 = plot(ax1, nan, nan, 'g-', 'LineWidth',2);
    hArc2 = plot(ax1, nan, nan, 'b-', 'LineWidth',2);
    hLineRef1 = plot(ax1, nan, nan, 'g--','LineWidth',1.5);
    hLineMov1 = plot(ax1, nan, nan, 'g-','LineWidth',1.5);
    hLineRef2 = plot(ax1, nan, nan, 'b--','LineWidth',1.5);
    hLineMov2 = plot(ax1, nan, nan, 'b-','LineWidth',1.5);
    ...
    % --- ANIMAZIONE CORRETTA ---
    h2 = plot(ax2, [P1_2(1,1) P2_2(1,1) P3_2(1,1)], [P1_2(1,2) P2_2(1,2) P3_2(1,2)], 'bo-','LineWidth',2,'MarkerFaceColor','b');
    ...
    hArc3 = plot(ax2, nan, nan, 'g-', 'LineWidth',2);
    hArc4 = plot(ax2, nan, nan, 'b-', 'LineWidth',2);
    hLineRef3 = plot(ax2, nan, nan, 'g--','LineWidth',1.5);
    hLineMov3 = plot(ax2, nan, nan, 'g-','LineWidth',1.5);
    hLineRef4 = plot(ax2, nan, nan, 'b--','LineWidth',1.5);
    hLineMov4 = plot(ax2, nan, nan, 'b-','LineWidth',1.5);
    hSpallaLabel2 = text(ax2,0,0,'','FontSize',9,'FontWeight','bold','Color','k','HorizontalAlignment','left');
    hGomitoLabel2 = text(ax2,0,0,'','FontSize',9,'FontWeight','bold','Color','k','HorizontalAlignment','left');
    hPolsoLabel2  = text(ax2,0,0,'','FontSize',9,'FontWeight','bold','Color','k','HorizontalAlignment','left');

    % --- CONTROLLI (GUI) ---
    uicontrol('Style','text','String','Frame:','Position',[20 170 50 20]);
    editFrame = uicontrol('Style','edit','String','1','Position',[80 170 60 20]);
    uicontrol('Style','text','String','Tempo [s]:','Position',[20 140 70 20]);
    editTime = uicontrol('Style','edit','String',sprintf('%.3f',t_min),'Position',[90 140 60 20]);
    btnGoTime = uicontrol('Style','pushbutton','String','Vai a tempo','Position',[160 140 80 20]);
    btnStart = uicontrol('Style','pushbutton','String','Start','Position',[20 110 60 25]);
    btnStop  = uicontrol('Style','pushbutton','String','Stop','Position',[90 110 60 25]);
    btnRestart = uicontrol('Style','pushbutton','String','Restart','Position',[160 110 60 25]);
    rangeStr = sprintf('Frame: 1-%d | Tempo: %.3f-%.3f s',N,t_min,t_max);
    uicontrol('Style','text','String',rangeStr,'Position',[20 70 400 25],'FontSize',11,'ForegroundColor','k');
    ...
    uitblErr = uitable('Data',tblDataErr,'ColumnName',varNames,...
        'Position',[60 860 670 70],'FontSize',10,'BackgroundColor',[1 0.95 0.95]);
    uitblCorr = uitable('Data',tblDataCorr,'ColumnName',varNames,...
        'Position',[920 860 670 70],'FontSize',10,'BackgroundColor',[0.95 0.95 1]);

    % --- GRAFICO PROGRESSIVO tau1 E tau2 ---
    cla(ax3);
    cla(ax4);
    ...
    function goToTime()
        ...
        % Se t1 non è strettamente crescente, usa find
        if any(diff(t1)<=0)
    ...
    while ishandle(fig)
        ...
        % Se campo tempo cambiato manualmente, aggiorna il frame
        ...
        [xa,ya,ang_c1] = arco(P1_1(frame,:), vRef1, vMov1, 0.07);
        ...
        [xa2,ya2,ang_c2] = arco(P2_1(frame,:), vRef2, vMov2, 0.05);
        ...
        [xa3,ya3,ang_c3] = arco(P1_2(frame,:), vRef3, vMov3, 0.07);
        ...
        [xa4,ya4,ang_c4] = arco(P2_2(frame,:), vRef4, vMov4, 0.05);
